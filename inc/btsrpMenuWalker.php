...
class Bootstrap_Walker_Menu_Nav extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		if ($depth == 0) $output .= '<ul class="dropdown-menu">';
	}
		...
		if (strcasecmp($item->title, '-') == 0) {
			...
			if ($depth == 0) {
				...
				if ($has_children)
				{
					...
					$attrs1 = 'data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"';
					$attrs2 = '<span class="caret"></span>';
				}
			} 
			else 
			{
			...
			$item_output .= $attrs3 . '<a'. $attributes . $attrs1 . '>';
			$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
			$item_output .= $attrs2 . '</a>' . $attrs4;
